...
\begin{figure}[h!]
\centering
\caption{Benchmark Results for forward algorithm}
<<Benchmark Plots, echo = F,  fig = T,fig.width=10, fig.height=6, out.width='\\textwidth'>>=
sizes <- expand.grid(K = 5:20, T = 5:20)
bench_results <- data.frame(K = integer(), T = integer(), time = numeric())

# Run benchmarks for different sizes and collect results
for (i in 1:nrow(sizes)) {
    ...
    benchmark <- mclapply(1:10, function(i) {
        system.time(forward(haps, hap, error = 0.1))[3]
    }, mc.cores = detectCores() - 1)
    
    # Collecting results
    mean_time <- mean(sapply(benchmark, `[[`, 1))
    bench_results <- rbind(bench_results, 
                           data.frame(K = K, T = T, time = mean_time))
}
...
# get plot colors
colors <- tableau_color_pal()(4)
cap <- "Median execution time required for varying values of K and T. Each point represents the median execution time across all 10 simulations for a given K or T. \nThe red and turquoise lines are smoothed fits indicating the trend of time complexity, with fitting quadratic in K and linear in T"

# plot
ggplot(bench_results_long, aes(x = value, y = time)) +
  stat_summary(
  ...
  geom_smooth(
    data = bench_results_long %>% filter(variable == "K"), 
    aes(x = value, y = time), 
    method = "lm", formula = y ~ poly(x, 2, raw = T), 
    color = colors[3], se = T
  ) +
  geom_smooth(
    data = bench_results_long %>% filter(variable == "T"), 
    aes(x = value, y = time), 
