...
new_packages <- c("knitr", "readr", "dplyr", "lubridate", "tidyr", "ggplot2", "ggthemes", "xtable")
...
# matrices α and β , both with K rows and T columns
forward <- function(haps, hap, error = 0.1) {
  ...
  # Calculate initial and emission probabilities
  pi <- 1 / K  # (eq. 1)
  for (k in 1:K) { # (eq. 4)
  ...
  transition_sum <- transition_sum + alpha[i, t - 1] * A_ik
      ...
      b_kt <- calculate_emission_probability(hap[t], haps[k, t], error)
